...
export function PainelEmpresa({ settings }: PainelEmpresaProps) {
  ...
  const sett = (tenant?.settings ?? {}) as { website?: string; tax_id?: string }
        ...
        {!editar && (
          <>
            <div className="space-y-4 mt-4">
              ...
              <Campo label="Identificação fiscal (NIF/CPF)" valor={sett.tax_id || '—'} />
        ...
        {editar && isOwner && (
          <form onSubmit={form.handleSubmit(onSubmit)} noValidate className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Input label="NIF / CPF / CNPJ" error={form.formState.errors.tax_id?.message} {...form.register('tax_id')} />
            </div>
